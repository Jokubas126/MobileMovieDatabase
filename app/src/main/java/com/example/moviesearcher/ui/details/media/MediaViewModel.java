...
public class MediaViewModel extends ViewModel {
    ...
    public void fetch(Activity activity, Bundle args){
        ...
        if (args != null){
            new MovieDbApiService().getTrailer(args.getInt(BundleUtil.KEY_MOVIE_ID),
                    video -> activity.runOnUiThread(() -> {
                        trailer.setValue((Video) video);
                        loading.setValue(false);
                    }));
            new MovieDbApiService().getImages(args.getInt(BundleUtil.KEY_MOVIE_ID),
    ...
    LiveData<Video> getTrailer() {
        return trailer;
    }

    LiveData<List<String>> getPosterList() {
        return posterList;
    }

    LiveData<List<String>> getBackdropList() {
        return backdropList;
    }
