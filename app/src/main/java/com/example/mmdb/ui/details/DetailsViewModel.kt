    ...
    val onMenuItemSelected = { menuItem: MenuItem ->
        when (menuItem.itemId) {
            R.id.overview_menu_item ->
                config.onBottomNavigationAction.invoke(InnerDetailsAction.OverviewAction(action.movieId))
            R.id.media_menu_item ->
                config.onBottomNavigationAction.invoke(InnerDetailsAction.MediaAction(action.movieId))
            R.id.credits_menu_item ->
                config.onBottomNavigationAction.invoke(InnerDetailsAction.CreditsAction(action.movieId))
        }
    }
}